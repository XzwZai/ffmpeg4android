...
int H264Publisher::StartPublish() {
    ...
    if (avformat_alloc_output_context2(&out_fmt, NULL, "flv", outputPath) < 0) {
        return -1;
    }
    //4.查找H.264编码器
...
int H264Publisher::EncodeBuffer(unsigned char *nv21Buffer) {
...
//                          libyuv::FOURCC_NV21);
    int ret = libyuv::ConvertToI420(nv21Buffer, width * height, i420_y, height, i420_u, height / 2, i420_v,
    ...
    //编码H.264
    ret = EncodeFrame(pCodecCtx, pFrame, &avPacket);
    return ret;
}
